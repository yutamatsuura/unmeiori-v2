...
function comprehensiveTest() {
    console.log("九星気学ロジック包括的検証開始\n");
    console.log("=" * 60);

    // 様々な年代のテストケース
    const testCases = [
        { date: LocalDate.of(1990, 5, 15), label: "平成初期" },
        { date: LocalDate.of(2000, 1, 1), label: "ミレニアム" },
        { date: LocalDate.of(2023, 12, 25), label: "令和現在" },
        { date: LocalDate.of(1945, 8, 15), label: "昭和戦後" },
        { date: LocalDate.of(1970, 3, 10), label: "昭和中期" },
        { date: LocalDate.of(2025, 2, 4), label: "節分直後" },
        { date: LocalDate.of(2025, 2, 3), label: "節分当日" },
        { date: LocalDate.of(1989, 12, 31), label: "平成前夜" },
    ];

    let results = [];

    testCases.forEach((testCase, index) => {
        console.log(`\\nテストケース${index + 1}: ${testCase.label} (${testCase.date.toString()})`);
        console.log("-" * 50);

        try {
            const yearQsei = Qsei.getYear(testCase.date);
            const monthQsei = Qsei.getMonth(testCase.date);
            const qseiDate = QseiDate.of(testCase.date);

            console.log(`年盤九星: ${yearQsei.name} (${yearQsei.index})`);
            console.log(`月盤九星: ${monthQsei.name} (${monthQsei.index})`);
            console.log(`九星暦年: ${qseiDate.year}`);
            console.log(`九星月索引: ${qseiDate.monthIndex}`);
            console.log(`年盤吉方位: [${yearQsei.findKipous().join(', ')}]`);
            console.log(`月盤吉方位: [${monthQsei.findKipous().join(', ')}]`);

            // 節入り確認
            const setu = Setu.enters(testCase.date.getYear());
            console.log(`立春: ${setu[0].toString()}`);

            results.push({
                date: testCase.date.toString(),
                label: testCase.label,
                year: yearQsei.index,
                month: monthQsei.index,
                qseiYear: qseiDate.year,
                monthIndex: qseiDate.monthIndex
            });
    ...
    results.forEach((result, index) => {
        if (result.error) {
            console.log(`${index + 1}. ${result.label}: エラー - ${result.error}`);
        } else {
            console.log(`${index + 1}. ${result.label}: 年${result.year}星 月${result.month}星`);
        }
    });
    ...
    for (let year = 1990; year <= 2000; year++) {
        const testDate = LocalDate.of(year, 5, 15);
        const yearQsei = Qsei.getYear(testDate);
        const mod = year % 9;
        const calculated = Qsei.getYearSub(year);
        console.log(`${year}年: ${yearQsei.index}星 (mod9=${mod}, calc=${calculated})`);
    }
...
// メイン実行
try {
    const results = comprehensiveTest();
    console.log("\\n検証完了。");
} catch (error) {
