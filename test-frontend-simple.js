...
async function testFrontendSimple() {
  ...
  try {
    ...
    // 計算ボタンを探してクリック
    const calculateButton = await page.$('button:contains("計算"), button[type="submit"]');
    if (calculateButton) {
      ...
      console.log(`✅ 五行関連: ${hasGogyouBalance ? '表示' : '未表示'}`);
      console.log(`✅ 陰陽関連: ${hasYouinPattern ? '表示' : '未表示'}`);

      // スクリーンショット撮影
      await page.screenshot({
        path: './tests/screenshots/simple-test-result.png',
        fullPage: true
      });
